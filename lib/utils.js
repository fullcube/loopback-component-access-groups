...
module.exports = class AccessUtils {
  constructor(app, options) {
    ...
    winston.debug('options: %o', options);
  }
   ...
   */
  setupFilters() {
    winston.debug('Setting up access filters');
  ...
  attachAccessObserver(modelName) {
    ...
    winston.debug('Attaching access observer to %s', modelName);
    Model.observe('access', (ctx, next) => {
      ...
      if (currentUser) {
        ...
        if (ctx.options.skipAccess) {
          winston.debug('skipAccess: true - skipping access filters');
          return next();
        }
        ...
        if (groupAccessApplied) {
          winston.debug('acls not appled - skipping access filters');
          return next();
        }
        ...
        return this.buildFilter(currentUser.getId(), ctx.Model)
          .then(filter => {
            ...
            winston.debug('modified query: %s', JSON.stringify(ctx.query, null, 4));
          });
      }
      return next();
    });
  ...
  attachBeforeSaveObserver(modelName) {
    ...
    winston.debug('Attaching before save observer to %s', modelName);
    Model.observe('before save', (ctx, next) => {
      if (!ctx.isNewInstance) {
        winston.debug('updating %s with params - %o', modelName, ctx.data);
        return next();
      }

      winston.debug('original object before save %o', ctx.instance);
  ...
  buildFilter(userId, Model) {
    winston.debug('Appending filter');
    const filter = { };
    ...
    return this.getUserGroups(userId)
      .then(userGroups => {
        winston.debug('original userGroups: %o', userGroups);
        const userGroupIds = Array.from(userGroups, group => group[this.options.foreignKey]);

        filter[key] = { inq: userGroupIds };

        winston.debug('where filter built from userGroupIds: %o - %o', userGroupIds, filter);
        return filter;
      });
  }
  ...
  getGroupContentModels() {
    ...
    winston.debug('Got group content models: %o', this.options.groupContentModels);
    return this.options.groupContentModels;
  }
  ...
  getUserGroups(userId, force, cb) {
    ...
    if (!force && currentUser && currentUser.getId() === userId) {
      winston.debug('getUserGroups returning from cache: %o', currentUserGroups);
    ...
    this.app.models[this.options.groupAccessModel].find({
      ...
      .then(groups => {
        winston.debug('getUserGroups returning from datastore: %o', groups);
        cb(null, groups);
      })
      .catch(cb);
  ...
  setupRoleResolver(accessGroup) {
    ...
    Role.registerResolver(accessGroup, (role, context, cb) => {
      ...
      if (!this.options.applyToStatic) {
        winston.debug('%o', context);
        if (!context || !modelClass || !modelId) {
          process.nextTick(() => {
            winston.debug('Deny access (context: %s, context.model: %s, context.modelId: %s)',
        ...
        this.isGroupMemberWithRole(modelClass, modelId, userId, roleName)
          .then(res => {
            ...
            winston.debug('Passed the ACL: %o', res);
      ...
      Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        (currentGroupId, targetGroupId) => {
          winston.debug('currentGroupId - %s, targetGroupId - %s', currentGroupId, targetGroupId);
          if (!currentGroupId) {
        ...
        .spread((currentGroupCount, targetGroupCount) => {
          ...
          if (currentGroupCount === false) {
            ...
            winston.info("Was unable to determine group content so allow access");
            res = true;
          }
          else {
  ...
  isGroupMemberWithRole(modelClass, modelId, userId, roleId, cb) {
    cb = cb || createPromiseCallback();
    winston.debug('isGroupMemberWithRole: modelClass: %o, modelId: %o, userId: %o, roleId: %o',
    ...
    if (this.isGroupModel(modelClass)) {
      winston.debug('Access to Group Model %s attempted', modelId);
    ...
    modelClass.findById(modelId, (err, inst) => {
      if (err || !inst) {
        winston.debug('Model not found for id %j', modelId);
        return cb(err, false);
      }
      winston.debug('Model found: %j', inst);
      ...
      winston.debug('No matching belongsTo relation found for model %j and group: %j', modelId, groupId);
  ...
  hasRoleInGroup(userId, roleName, groupId, cb) {
    winston.debug('hasRoleInGroup: role: %o, groupId: %o, userId: %o', roleName, groupId, userId);
    cb = cb || createPromiseCallback();
    const GroupAccess = this.app.models[this.options.groupAccessModel];
    const conditions = {
      [this.options.userIdKey]: userId,
  ...
  getCurrentGroupId(context, cb) {
    ...
    // Otherwise, return null.
      ...
      winston.debug('unable to determine current group context');
      process.nextTick(() => cb(null, groupId));
    }
  ...
  getTargetGroupId(context, cb) {
    ...
    // Otherwise, return null.
      ...
      winston.debug('unable to determine target group context');
    }
  ...
  setupEntityRoleResolver(accessGroup) {
    winston.debug(`Registering entity role resolver for ${accessGroup}`)
    const Role = this.app.models[this.options.roleModel];
