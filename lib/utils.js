    ...
    tenantModels.forEach(modelName => {
      ...
      if (typeof Model.observe === 'function') {
        ...
        Model.observe('access', (ctx, next) => {
          debug('%s observe access: query=%o, options=%o, hookState=%o',
            Model.modelName, ctx.query, ctx.options, ctx.hookState);
          ...
          // TODO: Remove this hack once ctx.options is passing properly (above).
          ...
          if (currentUser) {
            this.buildFilter(currentUser.getId())
              .then(filter => {
                ...
                debug('where query modified to: %o', where);
