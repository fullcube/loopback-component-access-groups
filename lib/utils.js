...
module.exports = class AccessUtils {
    ...
    Model.observe('before save', (ctx, next) => {
      ...
      if (currentUser) {
        ...
        if (ctx.instance[foreignKey] && ctx.instance[foreignKey] !== currentUser[foreignKey]) {
          logger.error(`BEFORE SAVE observer - Object field ${foreignKey} - ${ctx.instance[foreignKey]} does not match currentUser - ${currentUser[foreignKey]}`);
        }
        ...
        if (foreignKeySet) {
          ...
          // TODO: We are NOT setting the orgId at moment - keep this code in when we are confident this works
  ...
  setupRoleResolver(accessGroup) {
    logger.debug(`ORG ROLE RESOLVER - Registering for ${accessGroup}`);
    const Role = this.app.models[this.options.roleModel];

    Role.registerResolver(accessGroup, (role, context, cb) => {
      logger.debug('ORG ROLE RESOLVER - Hitting role resolver for', context.modelName, context.method);
      ...
      logger.debug(`ORG ROLE RESOLVER - role: ${chalk.blue(accessGroup)} model: ${chalk.blue(modelName)} method: ${chalk.blue(method)} userId: ${chalk.blue(userId)} modelId: ${chalk.blue(modelId)} with remoting data: ${util.inspect(remotingData)}`);
      // No userId is present
      if (!userId) {
        process.nextTick(() => {
          logger.debug(`ORG ROLE RESOLVER - Deny access for anonymous user`);
          cb(null, false);
        });
        return cb.promise;
      }
      ...
      Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        (currentGroupId, targetGroupId) => {
          if (!currentGroupId) {
            ...
            logger.debug(`ORG ROLE RESOLVER - Could not get group id, skipping ACL check for ${chalk.blue(accessGroup)} on model ${chalk.blue(modelName)} for method ${chalk.bold(method)}`);
            return [ false ];
          }
        ...
        .spread((currentGroupCount, targetGroupCount) => {
          ...
          if (currentGroupCount === false) {
            logger.debug('ORG ROLE RESOLVER - No group context was determined, so allow passthrough access')
            ...
            logger.debug(`ORG ROLE RESOLVER - User ${userId} ${bAllowAccess ? 'has role' : 'does not have role'} ${roleName} in group ${scope.currentGroupId}`);
            ...
            if (scope.targetGroupId && scope.targetGroupId !== scope.currentGroupId) {
              ...
              logger.debug(`ORG ROLE RESOLVER - Attempting save into new target group, User ${userId} ${tMember ? 'has role' : 'does not have'} ${roleName} in target group ${scope.targetGroupId}`);
          ...
          // Note the fact that we are allowing access due to passing an ACL.
           if (bAllowAccess) {
            logger.debug('ORG ROLE RESOLVER - Allowing access due to passing an ACL');
            this.app.loopback.getCurrentContext().set('groupAccessApplied', true);
          }

          logger.debug(`ORG ROLE RESOLVER - ${accessGroup} ${modelName}.${method} ACL check: ${chalk.blue(bAllowAccess)}`);
  ...
  setupEntityRoleResolver(accessGroup) {
    logger.debug(`ENTITY ROLE RESOLVER - Registering entity role resolver for ${chalk.blue(accessGroup)}`)
    const Role = this.app.models[this.options.roleModel];

    Role.registerResolver(accessGroup, (role, context, cb) => {
      logger.debug(`ENTITY ROLE RESOLVER - Starting entity role resolver`);
      ...
      const entity    = context.modelName;
      const scope         = { };

      this.getCurrentGroupId(context, (groupId) => {
        return this.hasRoleInEntity(userId, roleName, entity, modelId, groupId, cb);
      });
      return cb.promise;
    });
  }

  hasRoleInEntity(userId, roleName, entity, entityId, groupId, cb) {
    cb = cb || createPromiseCallback();
    ...
    logger.debug(`hasRoleInEntity search conditions ${util.inspect(conditions)}`);

    EntityAccess.count(conditions)
      .then(count => {
        const res = count > 0;

        logger.debug(`ENTITY ROLE RESOLVER - User ${userId} ${res ? 'HAS' : 'DOES NOT HAVE'} ${roleName} role in entity ${entity} id ${entityId}`);
        cb(null, res);
      });
