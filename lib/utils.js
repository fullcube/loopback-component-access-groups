...
module.exports = class AccessUtils {
  ...
  setupRoleResolver(accessGroup) {
    winston.debug(`Registering role resolver for ${accessGroup}`);
    ...
    Role.registerResolver(accessGroup, (role, context, cb) => {
      ...
      winston.debug(`${accessGroup} ROLE RESOLVER - model: ${chalk.blue(modelName)} method: ${chalk.blue(method)} userId: ${chalk.blue(userId)}`)
      if (modelId) winston.debug(`${accessGroup} ROLE RESOLVER - modelId: ${chalk.blue(modelId)}`);
      // No userId is present
      if (!userId) {
        process.nextTick(() => {
          winston.debug(`Deny access for anonymous user`);
          cb(null, false);
        });
        return cb.promise;
      }
      ...
      Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        (currentGroupId, targetGroupId) => {
          if (!currentGroupId) {
            ...
            winston.debug(`Could not get group id, skipping ACL check for ${accessGroup} on model ${modelName} for method ${method}`);
            return [ false ];
          }
        ...
        .spread((currentGroupCount, targetGroupCount) => {
          ...
          if (currentGroupCount === false) {
            ...
            winston.debug(`user ${userId} ${res ? 'is a' : 'is not a'} ${roleName} of group ${scope.currentGroupId}`);
            ...
            if (scope.targetGroupId && scope.targetGroupId !== scope.currentGroupId) {
              ...
              winston.debug(`user ${userId} ${tMember ? 'is a' : 'is not a'} ${roleName} of group ${scope.targetGroupId}`);
              res = res && tMember;
            }
          }
          ...
          winston.debug(`${accessGroup} ${modelName}.${method} ACL check: ${chalk.blue(res)}`);
          return cb(null, res);
        })
        .catch(cb);
      return cb.promise;
  ...
  getCurrentGroupId(context, cb) {
    ...
    if (context.modelId) {
      winston.debug(`fetching group id for existing model with id: ${context.modelId}`);
      context.model.findById(context.modelId, { }, {
        ...
        .then(item => {
          ...
          if (item) {
            winston.debug(`determined group id ${item[this.options.foreignKey]} from existing model ${context.modelId}`);
    ...
    // If we are creating a new model, get the foreignKey from the incoming data.
      ...
      winston.debug(`determined current group id ${groupId} from incoming data`);
  ...
  getTargetGroupId(context, cb) {
    ...
    if (_get(context, `remotingContext.args.data[${this.options.foreignKey}]`)) {
      winston.debug(`determined target group id ${groupId} from incoming data`);
      groupId = context.remotingContext.args.data[this.options.foreignKey];
    }
  ...
  hasRoleInEntity(userId, roleName, entityName, entityId, groupId, cb) {
    ...
    EntityAccess.count(conditions)
      .then(count => {
        ...
        winston.debug(`User ${userId} ${res ? 'HAS' : 'DOESNT HAVE'} ${roleName} role in entity ${entityName} id ${entityId}`);
        cb(null, res);
      });
