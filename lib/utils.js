...
module.exports = class AccessUtils {
  constructor(app, options) {
    ...
    winston.debug(chalk.grey(`Initializing Access Control`));
    winston.debug(`Access Control Options: ${JSON.stringify(options)}`);
  }
   ...
   */
  setupRoleResolvers() {
    winston.debug(chalk.grey(`Setup Role Resolvers`));
   ...
   */
  setupFilters() {
    winston.debug(chalk.grey(`Setup Filters`));
  ...
  attachAccessObserver(modelName) {
    ...
    winston.debug(`ACCESS observer - Attaching access observer to ${_modelName}`);
    Model.observe('access', (ctx, next) => {

      const ctxJSON = JSON.stringify(ctx.query);
      const currentUser = this.getCurrentUser();

      winston.info('Observing access for', modelName);

      if (currentUser) {
        // Do not filter if options.skipAccess has been set.
        if (ctx.options.skipAccess) {
          winston.debug(`ACCESS observer - skipAccess: true, no filter applied`);
          return next();
        }

        // // Do not apply filters if no group access acls were applied.
        // TODO: Why are we skipping the access controls if ACL passes?  Why is that important?
        // TODO: BG: I'm changing this so we always apply the access filters no matter what
        ...
        if (groupAccessApplied) {
          winston.debug(`ACCESS observer groupAccessApplied: true - returning without adding ACCESS filter to query`);
          return next();
        }

        // winston.debug('%s observe access: query=%s, options=%o, hookState=%o',
        ...
        return this.buildFilter(currentUser.getId(), ctx.Model)
          .then(filter => {
            ...
            winston.info('appending to query:', filter);
            ...
            winston.debug(`ACCESS observer - Modified query for model ${_modelName}: ${modifiedQuery}`);
          });
      }
  ...
  attachBeforeSaveObserver(modelName) {
    ...
    winston.debug(`BEFORE SAVE observer - Attaching before save observer to ${modelName}`);
    Model.observe('before save', (ctx, next) => {
      ...
      winston.debug(`BEFORE SAVE observer - Original data before save ctx.instance ${util.inspect(ctx.instance)}`);
      winston.debug(`BEFORE SAVE observer - Original data before save ctx.data ${util.inspect(ctx.data)}`);
      ...
      if (currentUser) {
        // Do not filter if options.skipAccess has been set.
        if (ctx.options.skipAccess) {
          winston.debug(`BEFORE SAVE observer - skipAccess: true, don't update ${foreignKey} for ${modelName} ${JSON.stringify(ctx.query)}`);
          return next();
        }
        ...
        if (ctx.instance[foreignKey] && ctx.instance[foreignKey] !== currentUser[foreignKey]) {
          winston.warn(`BEFORE SAVE observer - Object field ${foreignKey} - ${ctx.instance[foreignKey]} does not match currentUser - ${currentUser[foreignKey]}`);
        }

        winston.debug(`BEFORE SAVE observer - Setting object field ${foreignKey} to ${currentUser[foreignKey]}`);
        if (foreignKey && currentUser[foreignKey]) {
          ctx.instance[foreignKey] = currentUser[foreignKey];
          winston.debug(`BEFORE SAVE observer - updated object ${util.inspect(ctx.instance)}`);
        } else {
          winston.debug(`BEFORE SAVE observer - unable to update object foreignKey`);
        }
  ...
  getGroupContentModels() {
    ...
    winston.debug(`Got group content models: ${this.options.groupContentModels}`);
    return this.options.groupContentModels;
  }
  ...
  getUserGroups(userId, force, cb) {
    ...
    if (currentUserGroups && Array.isArray(currentUserGroups)) {
      ...
      if (shouldAlert) {
        winston.error('!!! potential incorrect user group in', JSON.stringify(currentUserGroups));
        winston.error(`!!! current user id: ${currentUser && currentUser.getId()}, user id: ${userId}`);
      }
    }

    // Return from the context cache if exists.
    if (!force && currentUser && currentUser.getId() === userId) {
      winston.debug(`getUserGroups returning from ${chalk.blue('cache')}: ${JSON.stringify(currentUserGroups)}`);
    ...
    this.app.models[this.options.groupAccessModel].find({
      ...
      .then(groups => {
        ...
        winston.debug(`getUserGroups returning from ${chalk.blue('datastore')}: ${groupsJSON}`);
  ...
  setupRoleResolver(accessGroup) {
    winston.debug(`ROLE RESOLVER - Registering for ${accessGroup}`);
    const Role = this.app.models[this.options.roleModel];

    Role.registerResolver(accessGroup, (role, context, cb) => {
      winston.info('hitting role resolver for', context.modelName, context.method);
      ...
      winston.debug(`ROLE RESOLVER - role: ${chalk.blue(accessGroup)} model: ${chalk.blue(modelName)} method: ${chalk.blue(method)} userId: ${chalk.blue(userId)} modelId: ${chalk.blue(modelId)} with data: ${util.inspect(remotingData)}`);
      // No userId is present
      if (!userId) {
        process.nextTick(() => {
          winston.debug(`ROLE RESOLVER - Deny access for anonymous user`);
          cb(null, false);
        });
        return cb.promise;
      }
      ...
      // if (!this.options.applyToStatic) {
        // winston.debug(`${util.inspect(context)}`);
        ...
        // }
        //
        // winston.debug(`Role resolver for ${role}: evaluate ${modelClass.modelName} with id: ${modelId} for user: ${userId}`);
        ...
        //     }
        //
        //     winston.debug(`Passed the ACL: ${res}`, res);
        //
        //     return cb(null, res);
        //   })
        //   .catch(cb);
      ...
      Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        (currentGroupId, targetGroupId) => {
          if (!currentGroupId) {
            ...
            winston.debug(`ROLE RESOLVER - Could not get group id, skipping ACL check for ${chalk.blue(accessGroup)} on model ${chalk.blue(modelName)} for method ${chalk.bold(method)}`);
            return [ false ];
          }
        ...
        .spread((currentGroupCount, targetGroupCount) => {
          ...
          if (currentGroupCount === false) {
            winston.info('No group context was determined, so allow passthrough access')
            ...
            winston.debug(`ROLE RESOLVER - User ${userId} ${bAllowAccess ? 'has role' : 'does not have role'} ${roleName} in group ${scope.currentGroupId}`);
            ...
            if (scope.targetGroupId && scope.targetGroupId !== scope.currentGroupId) {
              ...
              winston.debug(`ROLE RESOLVER - Attempting save into new target group, User ${userId} ${tMember ? 'has role' : 'does not have'} ${roleName} in target group ${scope.targetGroupId}`);
          ...
          // Note the fact that we are allowing access due to passing an ACL.
           if (bAllowAccess) {
            winston.debug('ROLE RESOLVER - Allowing access due to passing an ACL');
            this.app.loopback.getCurrentContext().set('groupAccessApplied', true);
          }

          winston.debug(`${accessGroup} ${modelName}.${method} ACL check: ${chalk.blue(bAllowAccess)}`);
  ...
  isGroupMemberWithRole(modelClass, modelId, userId, roleId, cb) {
    cb = cb || createPromiseCallback();
    winston.debug(`isGroupMemberWithRole: modelClass: ${modelClass && modelClass.modelName}, modelId: ${modelId}, userId: ${userId}, roleId: ${roleId}`);
    ...
    if (this.isGroupModel(modelClass)) {
      winston.debug(`Access to Group Model ${modelId} attempted`);
    ...
    modelClass.findById(modelId, (err, inst) => {
      if (err || !inst) {
        winston.debug(`Model not found for id ${modelId}`);
        return cb(err, false);
      }
      winston.debug(`Model found: ${inst}`);
      ...
      for (const relName in modelClass.relations) {
        ...
        if (rel.type === 'belongsTo' && this.isGroupModel(rel.modelTo)) {
          winston.debug('Checking relation %s to %s: %j', relName, rel.modelTo.modelName, rel);
          return inst[relName](function processRelatedGroup(error, group) {
            if (!error && group) {
              winston.debug('Group found: %j', group.getId());
      ...
      winston.debug(`No matching belongsTo relation found for model ${modelId} and group: ${groupId}`);
  ...
  hasRoleInGroup(userId, roleName, groupId, cb) {
    winston.debug(`hasRoleInGroup: role: ${roleName}, groupId: ${groupId}, userId: ${userId}`);
    cb = cb || createPromiseCallback();
    const GroupAccess = this.app.models[this.options.groupAccessModel];
    const conditions = {
      [this.options.userIdKey]: userId,
    ...
    GroupAccess.count(conditions)
      .then(count => {
        ...
        winston.debug(`User ${userId} ${res ? 'HAS' : 'DOESNT HAVE'} ${roleName} role in groupId ${groupId}`);
        cb(null, res);
      });
  ...
  getCurrentGroupId(context, cb) {
    ...
    if (context.modelId) {
      winston.debug(`fetching group id for model ${context.modelName} with id: ${context.modelId} for method ${context.method}`);
      context.model.findById(context.modelId, { }, {
        ...
        .then(modelInstance => {
          ...
          if (modelInstance) {
            winston.debug(`Determined group id ${modelInstance[this.options.foreignKey]} from model ${context.modelName} with id ${context.modelId} for method ${context.method}`);
    ...
    // If we are creating a new model, get the foreignKey from the incoming data.
      ...
      winston.debug(`Determined current group id ${groupId} from remoting incoming data for model ${context.modelName} for method ${context.method}`);
  ...
  getTargetGroupId(context, cb) {
    ...
    if (_get(context, `remotingContext.args.data[${this.options.foreignKey}]`)) {
      winston.debug(`Determined target group id ${groupId} from incoming data for model ${modelName} for method ${methodName}`);
      groupId = context.remotingContext.args.data[this.options.foreignKey];
    }
  ...
  setupEntityRoleResolver(accessGroup) {
    winston.debug(`Registering entity role resolver for ${chalk.blue(accessGroup)}`)
    const Role = this.app.models[this.options.roleModel];
  ...
  hasRoleInEntity(userId, roleName, entityName, entityId, groupId, cb) {
    ...
    EntityAccess.count(conditions)
      .then(count => {
        ...
        winston.debug(`User ${userId} ${res ? 'HAS' : 'DOES NOT HAVE'} ${roleName} role in entity ${entityName} id ${entityId}`);
        cb(null, res);
      });
