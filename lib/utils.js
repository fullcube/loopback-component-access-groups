    ...
    Role.registerResolver(accessGroup, (role, context, cb) => {
      ...
      return Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        (currentGroupId, targetGroupId) => {
          ...
          // If this is an attempt to save the item into a new group, ensure the user has access to the target group.
        ...
        .spread((currentGroupCount, targetGroupCount) => {
          ...
          const tMember = typeof targetGroupCount === 'undefined' ? true : targetGroupCount > 0;
          ...
          return cb(null, scope.targetGroupId ? cMember && tMember : cMember);
        })
        .catch(cb);
    });
  }
