...
module.exports = class AccessUtils {
  ...
  attachBeforeSaveObserver(modelName) {
    ...
    Model.observe('before save', (ctx, next) => {
      ...
      if (currentUser) {
        ...
        // Do not filter if the foreign key already exists
        if (ctx.instance[this.options.foreignKey]) {
          debug('foreign key already exists - skipping access filters');
          return next();
        }

        debug('setting instance %s to %s',
  ...
  buildFilter(userId, Model) {
    ...
    return this.getUserGroups(userId)
      .then(userGroups => {
        ...
        debug('where filter built from userGroups: %o - %o', userGroups, filter);
        return filter;
      });
  }
  ...
  getGroupContentModels() {
    if (this.options.groupContentModels && this.options.groupContentModels.length) {
  ...
  hasRoleInGroup(userId, roleName, group, cb) {
    debug('hasRoleInGroup: role: %o, group: %o, userId: %o', roleName, group, userId);
    cb = cb || createPromiseCallback();
    const GroupAccess = this.app.models[this.options.groupAccessModel];
    const conditions = {
      [this.options.userIdKey]: userId,
    ...
    GroupAccess.count(conditions)
      .then(count => {
        ...
        debug(`User ${userId} ${res ? 'HAS' : 'DOESNT HAVE'} ${roleName} role in group ${group}`);
        cb(null, res);
      });
