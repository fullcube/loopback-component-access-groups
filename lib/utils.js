...
module.exports = class AccessUtils {
  ...
  attachAccessObserver(modelName) {
    ...
    Model.observe('access', (ctx, next) => {
      ...
      console.log('Observing access for', modelName);
      ...
      if (currentUser) {
        ...
        if (!groupAccessApplied) {
          ...
          console.log('ACLs not applied - skipping access filters');
          return next();
        }

        console.log('ACLs applied - modifying query', Model.modelName, JSON.stringify(ctx.query && ctx.query.where, null, 2))
        ...
        return this.buildFilter(currentUser.getId(), ctx.Model)
          .then(filter => {
            ...
            console.log('appending to query:', filter);
  ...
  getUserGroups(userId, force, cb) {
    ...
    if (currentUserGroups && Array.isArray(currentUserGroups)) {
      ...
      if (shouldAlert) {
        console.log('!!! potential incorrect user group in', currentUserGroups);
        console.log('!!! current user id:', currentUser && currentUser.getId());
    ...
    if (!force && currentUser && currentUser.getId() === userId) {
      ...
      console.log('getUserGroups returning from cache:', currentUserGroups);
    ...
    this.app.models[this.options.groupAccessModel].find({
      ...
      .then(groups => {
        ...
        console.log('getUserGroups returning from datastore:', groups);
        cb(null, groups);
      })
      .catch(cb);
  ...
  setupRoleResolver(accessGroup) {
    ...
    Role.registerResolver(accessGroup, (role, context, cb) => {
      console.log('hitting role resolver for', context.modelName, context.method);
      ...
      Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        ...
        .spread((currentGroupCount, targetGroupCount) => {
          ...
          if (currentGroupCount === false) {
            console.log('No group context was determined, so allow passthrough access')
          ...
          // Note the fact that we are allowing access due to passing an ACL.
          if (res) {
            console.log('Allowing access due to passing an ACL');
            this.app.loopback.getCurrentContext().set('groupAccessApplied', true);
          }
