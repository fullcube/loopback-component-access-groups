...
module.exports = class AccessUtils {
  ...
  setupEntityRoleResolver(accessGroup) {
    ...
    Role.registerResolver(accessGroup, (role, context, cb) => {
      ...
      const entityName    = context.modelName;
      const scope         = { };

      this.getCurrentGroupId(context, (groupId) => {
        return this.hasRoleInEntity(userId, roleName, entityName, modelId, groupId, cb);
      });
      return cb.promise;
    });
  }
  ...
  hasRoleInEntity(userId, roleName, entityName, entityId, groupId, cb) {
    ...
    EntityAccess.count(conditions)
      .then(count => {
        ...
        logger.debug(`ENTITY ROLE RESOLVER - User ${userId} ${res ? 'HAS' : 'DOES NOT HAVE'} ${roleName} role in entity ${entityName} id ${entityId}`);
        cb(null, res);
      });
