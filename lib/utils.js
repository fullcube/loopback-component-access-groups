...
module.exports = class AccessUtils {
  ...
  attachAccessObserver(modelName) {
    ...
    Model.observe('access', (ctx, next) => {
      ...
      winston.debug(`${chalk.green('Starting Access Observer')} - model ${_modelName} on query ${ctxJSON}`);
      const currentUser = this.getCurrentUser();

      if (currentUser) {
        // Do not filter if options.skipAccess has been set.
        if (ctx.options.skipAccess) {
          winston.debug(`skipAccess: true - returning without adding ACCESS filter to query`);
          return next();
        }
        ...
        return this.buildFilter(currentUser.getId(), ctx.Model)
          .then(filter => {
            ...
            winston.debug(`ACCESS observer modified query: ${modifiedQuery}`);
          });
      }
  ...
  attachBeforeSaveObserver(modelName) {
    ...
    winston.debug(`Attaching before save observer to ${modelName}`);
    Model.observe('before save', (ctx, next) => {
      ...
      winston.debug(`Original data before save ${ctx.instance}`);
      winston.debug(`Original data before save ${ctx.data}`);
      ...
      if (currentUser) {
        // Do not filter if options.skipAccess has been set.
        if (ctx.options.skipAccess) {
          winston.debug('skipAccess: true - skipping adding BEFORE SAVE filters for model ${ctx.Model}');
          return next();
        }
