...
module.exports = function accessLoggerMiddleware() {
  ...
  return function accessLogger(req, res, next) {
    ...
    res.on('finish', () => {
      ...
      winston.debug(`${chalk.cyan('Finish Request')} - ${req.method} ${req.originalUrl} lbHttpMethod: ${req.method} lbUrl ${req.originalUrl} lbStatusCode ${res.statusCode} lbResponseTime  ${duration} lbResponseTimeUnit: 'ms'`);
    });
