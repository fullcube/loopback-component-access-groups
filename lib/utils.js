  ...
  getUserGroups(userId, force, cb) {
    ...
    if (currentUserGroups && Array.isArray(currentUserGroups)) {
      let shouldAlert = currentUserGroups.find(group => {
        return group[this.options.foreignKey] === 158; // explicitly check for problem id
      });

      if (shouldAlert) {
        winston.error('!!! potential incorrect user group in', currentUserGroups);
        winston.error('!!! current user id:', currentUser && currentUser.getId());
        winston.error('!!! user id:', userId);
      }
    }
    ...
    if (!force && currentUser && currentUser.getId() === userId) {
      ...
      winston.info('getUserGroups returning from cache:', currentUserGroups);
    ...
    this.app.models[this.options.groupAccessModel].find({
      ...
      .then(groups => {
        ...
        winston.info('getUserGroups returning from datastore:', groups);
        cb(null, groups);
      })
      .catch(cb);
