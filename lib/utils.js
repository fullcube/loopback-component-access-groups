...
module.exports = class AccessUtils {
  ...
  getUserTenants(userId, force, cb) {
    force = force || false;
    ...
    if (!force && currentUser && currentUser.getId() === userId) {
      debug('getUserTenants returning from cache: %o', currentUserGroups);
    ...
    this.app.models[this.options.groupModel].find({
      ...
      .then(groups => {
        debug('getUserTenants returning from datastore: %o', currentUserGroups);
        cb(null, groups);
      })
      .catch(cb);
  ...
  getGroupId(context, cb) {
      ...
      groupId = context.model.findById(context.modelId, { }, {
        ...
        .then(item => {
          ...
          if (item) {
            ...
            return item[this.options.foreignKey];
          }
        });
    }
    ...
    // If we are creating a new model, get the foreignKey from the incoming data.
      ...
      debug(`determined group id ${groupId} from incoming data`);
